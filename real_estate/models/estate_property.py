...
class EstateProperty(models.Model):
    ...
    description = fields.Text(help="Description of the property")
    ...
    date_availability = fields.Date(
        help="Date when the property will be available", default=fields.Date.today
    )

    expected_price = fields.Float(
        required=True, help="Expected selling price of the property"
    )

    selling_price = fields.Float(
        readonly=True, copy=False, help="Final selling price of the property"
    )

    bedrooms = fields.Integer(
        string="Number of Bedrooms", help="The number of bedrooms in the property"
    )

    living_area = fields.Integer(
        string="Living Area (sqm)", help="The living area in square meters"
    )

    facades = fields.Integer(
        string="Number of Facades", help="The number of facades of the property"
    )

    garage = fields.Boolean(help="Whether the property has a garage or not")

    garden = fields.Boolean(help="Whether the property has a garden or not")

    garden_area = fields.Integer(
        string="Garden Area (sqm)", help="The area of the garden"
    )
    ...
    def _check_selling_price(self):
        for rec in self:
            if len(rec.offers_ids) > 1:
                if (
                    float_compare(
                        rec.selling_price, rec.expected_price * 0.9, precision_digits=2
                    )
                    == -1
                ):
                    raise ValidationError(
                        _(
                            "The selling price can't be lower than 90% of the expected price."
                        )
                    )
            elif len(rec.offers_ids) == 0 and rec.selling_price != 0:
                raise ValidationError(
                    _("The selling price must be 0 if there are no offers.")
                )
...
class EstatePropertyOffer(models.Model):
    ...
    price = fields.Float(required=True, help="Offered price for the property")
    ...
    validity = fields.Integer(default=7, help="Number of days the offer is valid")

    deadline_date = fields.Datetime(
        compute="_compute_deadline", string="Deadline", help="Deadline for the offer"
    )
    ...
    def _compute_deadline(self):
        for record in self:
            if record.create_date:
                ...
                record.deadline_date = fields.Datetime.now() + timedelta(
                    days=record.validity
                )
    ...
    def create(self, vals_list):
        for vals in vals_list:
            ...
            if offer_ids and vals["price"] < max(offer_ids.mapped("price"), default=0):
                raise UserError(
                    "A new can't be have a lower price than the highest current offer."
                )
            elif not offer_ids:
